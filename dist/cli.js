...
function effortArg(name, v, fallback) {
    if (v === undefined)
        return fallback;
    if (!isEffort(v)) {
...
async function main() {
    ...
    if (values['route-only']) {
        if (!prompt)
            throw new Error('--route-only needs a prompt');
    ...
    const run = async (p) => {
        ...
        try {
            ...
            if (values.json)
                console.log(JSON.stringify({ ...report, jevCostUsd }, null, 2));
            else {
                if (report.isError)
                    console.log(c.red(report.result));
    ...
    try {
        ...
        await session.close();
        if (values.verbose)
            notice(c.dim(`trace: ${trace.file}`));
    }
}
async function gateway(jev, bounds, port) {
    const trace = createTrace(config.traceDir);
 ...
 */
async function jevClient(disabled = false) {
    if (disabled)
        return null;
    const jev = new JevClient();
    if (jev.enabled || !process.stdin.isTTY || !process.stdout.isTTY || fs.existsSync(CONFIG_ENV_FILE))
        return jev;
...
async function repl(session, router, terminal, run, notice) {
    ...
    for (;;) {
        ...
        if (line.startsWith('/pin')) {
            ...
            if (!isEffort(e)) {
                notice(`usage: /pin ${EFFORT_LEVELS.join('|')}`);
                continue;
            }
        ...
        if (line.startsWith('/bounds')) {
            ...
            if (!isEffort(lo) || !isEffort(hi)) {
                notice('usage: /bounds <min> <max>');
                continue;
            }
...
async function doctor(router, jev, model, settings) {
    ...
    try {
        ...
        failures++;
        bad(`Claude Code not runnable at "${claudeBin}": ${err.message}`);
    }
    ...
    if (jev) {
        ...
        if (d.source === 'jev')
            ok(`Jev API: ${d.profile?.taskType}, difficulty ${d.profile?.difficulty.toFixed(1)} → ${d.effort} in ${d.jevLatencyMs}ms`);
        else {
            failures++;
            bad(`Jev API: ${d.jevError ?? 'no answer'}`);
        }
    }
    else {
    ...
    const session = new JevOpusSession({
        ...
        settingSources: (settings ?? 'project,local').split(','),
        claudePath: config.claudePath,
        maxTurns: 1,
    });
    try {
        ...
        if (r.isError) {
            failures++;
            bad(`Claude (${model}): ${r.result}`);
        }
        else
            ok(`Claude (${model}) answered "${r.result.trim().slice(0, 40)}" at effort low · $${r.costUsd.toFixed(4)}`);
    }
    catch (err) {
        failures++;
        bad(`Claude (${model}): ${err.message}`);
    }
    finally {
        await session.close();
    }
