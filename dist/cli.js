...
async function main() {
    ...
    if (argv[0] === 'claude') {
        ...
        if (!jev.enabled)
            console.error(c.yellow('JEV_API_KEY not set — routing with local heuristics (run `jev-opus init`).'));
    ...
    try {
        ...
        await session.close();
        if (values.verbose)
            console.log(c.dim(`trace: ${trace.file}`));
    }
}
...
async function repl(session, router, terminal, run) {
    ...
    for (;;) {
        ...
        if (line.startsWith('/pin')) {
            ...
            if (!isEffort(e)) {
                console.log(`usage: /pin ${EFFORT_LEVELS.join('|')}`);
                continue;
            }
        ...
        if (line.startsWith('/bounds')) {
            ...
            if (!isEffort(lo) || !isEffort(hi)) {
                console.log('usage: /bounds <min> <max>');
                continue;
            }
