...
export function heuristicTaskProfile(prompt) {
    const text = prompt.trim();
    let taskType = text.length < 60 ? 'chat' : 'code_small';
...
export function heuristicStepSignals(ctx) {
    ...
    if (calls.length === 0)
        ...
        phase = 'diagnosing';
    else if (calls.some((c) => WRITE_TOOLS.test(c.tool)))
        phase = 'implementing';
    else if (calls.some((c) => c.tool === 'Bash' && VERIFY_CMD.test(c.summary)))
        phase = 'verifying';
    else if (calls.every((c) => READ_ONLY_TOOLS.test(c.tool)))
        phase = 'exploring';
    const stepDifficulty = phase === 'diagnosing' ? Math.min(4, 2.2 + 0.5 * ctx.consecutiveFailures)
