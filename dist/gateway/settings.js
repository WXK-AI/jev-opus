 ...
 */
export const NARRATION_PROMPT = 'Before each tool call, write one short sentence (under 15 words) saying what you are about to do. Keep it to a single line.';
/** Append the narration instruction to the caller's --append-system-prompt, or add one. */
export function withNarration(args, prompt = NARRATION_PROMPT) {
    const out = [...args];
    for (let i = 0; i < out.length && out[i] !== '--'; i++) {
