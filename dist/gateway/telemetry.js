...
export class ResponseTelemetry {
    ...
    onTool;
    cap = 1024 * 1024;
    constructor(contentType, onTool = () => { }, onText = () => { }) {
    ...
    push(chunk) {
        ...
        for (let i = 0; i < chunk.length; i += 16384)
            this.consume(this.decoder.write(chunk.subarray(i, i + 16384)));
    }
    end() {
    ...
    consume(text) {
        ...
        while ((boundary = /\r?\n\r?\n/.exec(this.buffer))) {
            ...
            if (!this.dropping && frame.length <= this.cap)
                this.frame(frame);
            this.dropping = false;
        }
        if (this.buffer.length > this.cap) {
        ...
        try {
            ...
            if (e?.type === 'message_delta') {
                ...
                if (typeof e.delta?.stop_reason === 'string')
                    this.usage.stopReason = e.delta.stop_reason;
                if (typeof e.usage?.output_tokens === 'number')
                    this.usageComplete = true;
            }
            if (e?.type === 'content_block_delta' && e.delta?.type === 'text_delta' && typeof e.delta.text === 'string' && e.delta.text.trim())
                this.onText();
            if (e?.type === 'content_block_start' && e.content_block?.type === 'text' && e.content_block.text?.trim())
                this.onText();
            if (e?.type === 'content_block_start' && e.content_block?.type === 'tool_use' && typeof e.content_block.id === 'string')
                this.onTool(e.content_block.id);
    ...
    message(m) {
        ...
        if (typeof m.id === 'string')
            this.responseId = m.id;
        if (typeof m.model === 'string')
            this.usage.model = m.model;
        if (typeof m.stop_reason === 'string')
            this.usage.stopReason = m.stop_reason;
        this.merge(m.usage);
    ...
    merge(raw) {
        if (!raw || typeof raw !== 'object')
            return;
        const fields = { input_tokens: 'inputTokens', output_tokens: 'outputTokens', cache_read_input_tokens: 'cacheReadInputTokens', cache_creation_input_tokens: 'cacheCreationInputTokens' };
        for (const [from, to] of Object.entries(fields)) {
            if (typeof raw[from] === 'number' && Number.isFinite(raw[from]) && raw[from] >= 0)
                this.usage[to] = raw[from];
        }
    }
}
