 ...
 */
export const envFileVars = new Map<string, { value: string; file: string }>();
for (const file of [path.join(PROJECT_ROOT, '.env'), CONFIG_ENV_FILE]) {
  ...
  for (const [k, v] of Object.entries(parsed)) {
    ...
    if (v !== '' && !envFileVars.has(k)) envFileVars.set(k, { value: v, file });
...
const noCredential = (): ClaudeCredential => ({ value: '', source: '' });
...
export function resolveClaudeCredentials(
  ...
  fileVars: ReadonlyMap<string, { value: string; file: string }>,
  opts: { inherit?: boolean } = {},
): ClaudeCredentials {
  const pick = (jevName: string, plainName?: string): ClaudeCredential => {
    const direct = env[jevName];
    if (direct) return { value: direct, source: jevName };
    if (plainName) {
      const fromFile = fileVars.get(plainName);
      if (fromFile?.value) return { value: fromFile.value, source: fromFile.file };
